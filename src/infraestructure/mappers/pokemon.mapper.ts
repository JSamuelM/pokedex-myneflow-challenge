...
import type { PokeAPIPokemon } from '../interfaces/pokemons.interface';
...
export class PokemonMapper {
  ...
  static async pokeApiPokemonToEntity(data: PokeAPIPokemon): Promise<Pokemon> {
    ...
    // third party, get color from Image
    // const color = getColorFromImage(avatar);
    ...
    return {
      ...
      color: '#ffffff',
      games: data.game_indices.map(gameIndex => gameIndex.version.name),
  ...
  static getSprites(data: PokeAPIPokemon): string[] {
    const sprites: string[] =[
      ...
      data.sprites.front_shiny, // shiny
